...
So, to render a 1, only segments c and f would be turned on; the rest would be off. To render
a 7, only segments a, c, and f would be turned on.
...
g, but those wires are connected to segments randomly. Worse, the wire/segment connections are
mixed up separately for each four-digit display! (All of the digits within a display use the
...
and f are meant to be on. With just that information, you still can't tell which wire (b/g) goes
...
the connections actually are). The unique signal patterns correspond to the ten different ways
the submarine tries to render a digit using the current wire/segment connections. Because 7 is
the only digit that uses three segments, dab in the above example means that to render a 7, signal
...
func SevenSegmentSearch(input string) int {
	...
	for scanner.Scan() {
		entry := scanner.Text()
		entrySplit := r.Split(string(entry), -1)
		signalPattern := r2.Split(strings.Trim(entrySplit[0], " "), -1)
		outputValue := r2.Split(strings.Trim(entrySplit[1], " "), -1)
		for _, value := range outputValue {
			if (len(value) >= 2 && len(value) <= 4) || len(value) == 7 {
				for _, signal := range signalPattern {
					if len(value) == len(signal) {
						count++
						break
					}
				}
...
func SevenSegmentSearch_Part2(input string) int {
	inputFile, err := os.Open(input)
	if err != nil {
		return -1
	}
	...
	for scanner.Scan() {
		...
		// build unique patterns, start with the easy digits
		var uniquePatterns [10]string
		fiveLetterPatterns := make([]string, 0)
		sixLetterPatterns := make([]string, 0)
		for _, pattern := range signalPattern {
			if (len(pattern) >= 2 && len(pattern) <= 4) || len(pattern) == 7 {
				// digit:segments = 1:2 4:4 7:3 8:7
				switch len(pattern) {
				case 2:
					uniquePatterns[1] = pattern
				case 3:
					uniquePatterns[7] = pattern
				case 4:
					uniquePatterns[4] = pattern
				case 7:
					uniquePatterns[8] = pattern
				}
			} else if len(pattern) == 5 {
				fiveLetterPatterns = append(fiveLetterPatterns, pattern)
			} else {
				// length should be 6
				sixLetterPatterns = append(sixLetterPatterns, pattern)
			}
		}

		for _, pattern := range fiveLetterPatterns {
			// find matching charaters 7
			//   if match 2 -> 3
			counter := 0
			for _, char := range strings.Split(pattern, "") {
				if !strings.Contains(uniquePatterns[7], char) {
					counter++
				}
			}
			if counter == 2 {
				uniquePatterns[3] = pattern
			} else {
				// count characters not in 4
				//   if 2 remain -> 5
				//   if 3 remain -> 2
				counter = 0
				for _, char := range strings.Split(pattern, "") {
					if !strings.Contains(uniquePatterns[4], char) {
						counter++
					}
				}
				if counter == 3 {
					uniquePatterns[2] = pattern
				} else {
					// counter should be 2
					uniquePatterns[5] = pattern
				}
			}
		}

		for _, pattern := range sixLetterPatterns {
			// find matching charaters 4
			//   if match 4 -> 9
			//   if 3 remain -> 0 or 6
			counter := 0
			for _, char := range strings.Split(uniquePatterns[4], "") {
				if strings.Contains(pattern, char) {
					counter++
				}
			}
			if counter == 4 {
				uniquePatterns[9] = pattern
			} else {
				// counter should be 3
				// count characters from 7
				//   if 1 remain -> 6
				//   if 0 remain -> 0
				counter = 0
				for _, char := range strings.Split(uniquePatterns[7], "") {
					if !strings.Contains(pattern, char) {
						counter++
					}
				}
				if counter == 1 {
					uniquePatterns[6] = pattern
				} else {
					uniquePatterns[0] = pattern
				}
			...
			if (len(value) >= 2 && len(value) <= 4) || len(value) == 7 {
				...
				for i, pattern := range uniquePatterns {
					if len(value) == len(pattern) {
						...
						sort.Strings(tmpValue)
						tmpSignal := strings.Split(pattern, "")
						sort.Strings(tmpSignal)

						if strings.Join(tmpValue, "") == strings.Join(tmpSignal, "") {
							outputStr = outputStr + strconv.Itoa(i)
						}
					}
				}
			}
		}
		tmpNum, _ := strconv.Atoi(outputStr)
		output += tmpNum
	}
	//fmt.Println(output)
	return output
