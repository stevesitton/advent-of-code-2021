...
func BinaryDiagnostics(input string) int {
	...
	diagnosticCount := make(map[int][]int)
	count := 0
	for scanner.Scan() {
		entry := scanner.Text()
		for i := 0; i < len(entry); i++ {
			value := string(entry[i])
			values := diagnosticCount[i]
			if values == nil {
				count++
				values = []int{0, 0}
			}
			...
			diagnosticCount[i] = values
		}
	}
	gammerNumberStr, eplisonRateStr := "", ""

	for i := 0; i < len(diagnosticCount); i++ {
		item := diagnosticCount[i]
		if item[0] > item[1] {
			gammerNumberStr += "0"
			eplisonRateStr += "1"
		} else {
	...
	gammerNumberBinary, _ := strconv.ParseInt(gammerNumberStr, 2, 64)
	eplisonRateBinary, _ := strconv.ParseInt(eplisonRateStr, 2, 64)
	//fmt.Println(int(gammerNumberBinary) * int(eplisonRateBinary))
	return int(gammerNumberBinary) * int(eplisonRateBinary)
}
...
func BinaryDiagnostics_Part2(input string) int {
	inputFile, err := os.Open(input)
	if err != nil {
		return -1
	}
	...
	scanner := bufio.NewScanner(inputFile)
	oxygenRatingNumbers := make([]string, 0)
	co2RatingNumbers := make([]string, 0)
	for scanner.Scan() {
		entry := scanner.Text()
		oxygenRatingNumbers = append(oxygenRatingNumbers, entry)
		co2RatingNumbers = append(co2RatingNumbers, entry)
	}

	oxygenRatingNumber := GetRatingNumber(oxygenRatingNumbers, true)
	co2RatingNumber := GetRatingNumber(co2RatingNumbers, false)

	oxygenRatingNumberBinary, _ := strconv.ParseInt(oxygenRatingNumber, 2, 64)
	co2RatingNumberBinary, _ := strconv.ParseInt(co2RatingNumber, 2, 64)

	//fmt.Println(int(oxygenRatingNumberBinary) * int(co2RatingNumberBinary))
	return int(oxygenRatingNumberBinary) * int(co2RatingNumberBinary)
}

func GetRatingNumber(slice []string, isMostPopular bool) string {
	pointer := 0
	for {
		if len(slice) == 1 {
			break
		}
		bit := GetMostPopularBit(slice, pointer)
		if !isMostPopular {
			if bit == 0 {
				bit = 1
			} else {
		...
		slice = ReduceSlice(slice, pointer, remove)
		pointer++
	}
	return slice[0]
}

func GetMostPopularBit(slice []string, pointer int) int {
	zero, one := 0, 0
	for _, item := range slice {
		if string(item[pointer]) == "0" {
			zero++
		} else {
			one++
		}
...
func ReduceSlice(slice []string, pointer int, remove string) []string {
	output := make([]string, 0)
	for _, item := range slice {
		if string(item[pointer]) != remove {
