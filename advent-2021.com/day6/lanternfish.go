...
After  2 days: 1,2,1,6,0,8 = 6
After  3 days: 0,1,0,5,6,7,8 = 7
After  4 days: 6,0,6,4,5,6,7,8,8 = 9
After  5 days: 5,6,5,3,4,5,6,7,7,8 = 10
After  6 days: 4,5,4,2,3,4,5,6,6,7 = 10
After  7 days: 3,4,3,1,2,3,4,5,5,6 = 10
After  8 days: 2,3,2,0,1,2,3,4,4,5 = 10
After  9 days: 1,2,1,6,0,1,2,3,3,4,8 = 11
After 10 days: 0,1,0,5,6,0,1,2,2,3,7,8 = 12
After 11 days: 6,0,6,4,5,6,0,1,1,2,6,7,8,8,8 = 15
After 12 days: 5,6,5,3,4,5,6,0,0,1,5,6,7,7,7,8,8 = 17
After 13 days: 4,5,4,2,3,4,5,6,6,0,4,5,6,6,6,7,7,8,8 = 19
After 14 days: 3,4,3,1,2,3,4,5,5,6,3,4,5,5,5,6,6,7,7,8 = 20
After 15 days: 2,3,2,0,1,2,3,4,4,5,2,3,4,4,4,5,5,6,6,7 = 20
After 16 days: 1,2,1,6,0,1,2,3,3,4,1,2,3,3,3,4,4,5,5,6,8 = 21
After 17 days: 0,1,0,5,6,0,1,2,2,3,0,1,2,2,2,3,3,4,4,5,7,8 = 22
...
days?
*/
func Lanternfish(input string, days int) int {
	...
	r, _ := regexp.Compile(",")
	bodySplit := r.Split(string(body), -1)
	state := make([]int, 0)
	for _, item := range bodySplit {
...
func Lanternfish_Part2(input string, days int) int {
	inputFile, err := os.Open(input)
	if err != nil {
		return -1
	}
	...
	for {
		...
		counter++
		//PrintState(counter, state)
	}
	fmt.Println(CountLanterns(state))
	return CountLanterns(state)
}

func PrintState(day int, state [9]int) {
	fmt.Printf("**DAY %v** %v+%v+%v+%v+%v+%v+%v+%v+%v=%v\n",
		day, state[0], state[1], state[2], state[3], state[4], state[5], state[6], state[7], state[8],
		CountLanterns(state))
}

func CountLanterns(state [9]int) int {
