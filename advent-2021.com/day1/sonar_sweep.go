...
the previous sum? */
func SonarSweep_Part2(input string) int {
	reportFile, err := os.Open(input)
	if err != nil {
		return -1
	}
	...
	for scanner.Scan() {
		...
		if len(measurement) < 3 {
			measurement = append(measurement, entry)
		} else {
			// sum numbers in slice
			measure1, measure2 := 0, 0
			for i, num := range measurement {
				if i > 0 {
					measure2 += num
				}
				measure1 += num
