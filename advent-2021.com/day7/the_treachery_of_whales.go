...
func TheTreacheryOfWhales(input string) int {
	...
	for _, num := range positions {
		...
		for _, crab := range positions {
			moves += math.Abs(float64(num) - float64(crab))
		}
		if int(moves) < lowest {
			lowest = int(moves)
		}
...
func TheTreacheryOfWhales_Part2(input string) int {
	inputFile, err := os.Open(input)
	if err != nil {
		return -1
	}
	...
	for i := range positions {
		...
		for _, crab := range positions {
			fuel := 1
			var crabMoves float64 = math.Abs(float64(i) - float64(crab))
			for i := 0; i < int(crabMoves); i++ {
				moves += fuel
				fuel++
			}
		}
