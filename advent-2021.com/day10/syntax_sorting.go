...
or more chunks on each line, and chunks contain zero or more other chunks. Adjacent chunks
are not separated by any delimiter; if one chunk stops, the next chunk (if any) can
...
score for syntax errors in a file? It's true! To calculate the syntax error score for a
line, take the first illegal character on the line and look it up in the following table:
...
Find the first illegal character in each corrupted line of the navigation subsystem. What
is the total syntax error score for those errors? */
func SyntaxSorting(input string) int {
	...
	for scanner.Scan() {
		...
		for _, item := range lineArr {
			if len(buffer) == 0 {
				buffer = append(buffer, item)
			} else if item == "]" || item == ">" || item == "}" || item == ")" {
				if buffer[len(buffer)-1]+item == "()" ||
					buffer[len(buffer)-1]+item == "<>" ||
					buffer[len(buffer)-1]+item == "{}" ||
					buffer[len(buffer)-1]+item == "[]" {
					...
					case "]":
						score += 57 // ]: 57 points
					case "}":
						score += 1197 // }: 1197 points
					case ">":
						score += 25137 // >: 25137 points
					}
					break
				}
			} else {
...
func SyntaxSorting_Part2(input string) int {
	inputFile, err := os.Open(input)
	if err != nil {
		return -1
	}
	...
	for scanner.Scan() {
		...
		if incomplete {
			...
			completeScores = append(completeScores, score)
		}
	}
	sort.Sort(sort.Reverse(sort.IntSlice(completeScores)))
	//fmt.Println(completeScores[len(completeScores)/2])
